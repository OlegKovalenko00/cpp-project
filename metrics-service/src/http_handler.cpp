...
HttpHandler::HttpHandler(int port, DatabaseCheckFunc db_check)
    : port_(port), db_check_(db_check), impl_(std::make_unique<Impl>()) {
    ...
    impl_->server.Get("/health/ready", [this](const httplib::Request& req, httplib::Response& res) {
        bool db_connected = db_check_ ? db_check_() : false;
        std::string timestamp = getCurrentTimestamp();
        std::string status = db_connected ? "ready" : "not_ready";
        std::string response = R"({"status":")" + status + R"(","database_connected":)" + 
                              (db_connected ? "true" : "false") + R"(,"timestamp":")" + timestamp + R"("})";
        
        res.status = db_connected ? 200 : 503;
        res.set_content(response, "application/json");
        std::cout << "[HTTP] GET /health/ready -> " << res.status 
                  << (db_connected ? " (DB connected)" : " (DB disconnected)") << std::endl;
    });
